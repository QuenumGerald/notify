...
func TestIgniteNotify(t *testing.T) {
	var (
		require = require.New(t)
		env     = envtest.New(t)
		app     = env.Scaffold("ignite-notify-app")
	)

	dir, err := os.Getwd()
	require.NoError(err)
	pluginPath := filepath.Join(filepath.Dir(filepath.Dir(dir)), "ignite-notify")

	env.Must(env.Exec("install ignite-notify app locally",
	...
	buf := &bytes.Buffer{}
	env.Must(env.Exec("run ignite-notify",
		step.NewSteps(step.New(
			step.Exec(
				envtest.IgniteApp,
				"ignite-notify",
